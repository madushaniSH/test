...
if($row_count == 0){
    ...
    if($valid_upload){
        if(move_uploaded_file($_FILES["manu_logo"]["tmp_name"], $image_file)){
            ...
            $stmt = $pdo->prepare($sql);
            $stmt->execute(['manufacturer_name'=>$_POST['manufacturer_name'], 'manufacturer_local_name'=>$_POST['manufacturer_local_name'], 'manufacturer_source'=>$_POST['manufacturer_source'], 'manufacturer_image_location'=>$image_file]);
