...
if ($_POST['type'] == 'probe') {
    ...
    $sql = 'SELECT SUBSTRING_INDEX(products.product_name, \' \', 1 ) as name FROM probe_qa_queue INNER JOIN products ON probe_qa_queue.product_id = products.product_id INNER JOIN ref_product_info ON ref_product_info.product_id = products.product_id INNER JOIN reference_info ON reference_info.reference_info_id = ref_product_info.reference_info_id   WHERE products.product_type = :product_type AND (probe_qa_queue.probe_being_handled = 0 OR probe_qa_queue.account_id = :account_id) AND reference_info.reference_ticket_id = :ticket AND products.product_hunt_type = :type';
    $stmt = $pdo->prepare($sql);
    $stmt->execute(['product_type'=>$_POST['product_type'], 'account_id' =>$_SESSION['id'], 'ticket'=>$_POST['ticket'], "type"=>$_POST['type']]);
    $brand_rows = $stmt->fetchAll(PDO::FETCH_OBJ);
}
